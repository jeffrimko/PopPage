...
== Introduction
PopPage is a simple command-line utility for applying content to {txt_jintmp} to generate files and directories. This project originally started as a static website generator but has since grown into something of a partial https://github.com/audreyr/cookiecutter[cookiecutter] clone. The main features of PopPage are:
